#!/usr/bin/env python3
"""Compare Stage-1 GS normals with per-frame Blender world-space EXR normals."""
...
def main() -> None:
    ...
    with torch.no_grad():
        ...
        for camera, record in camera_records:
            ...
            if gaussians.use_photometric_normal:
                ...
                normal_source = "independent_photometric_normal"
            else:
                gs_normal = F.normalize(rendered["rend_normal"], dim=0)
                normal_source = "gs_raster_normal"
        ...
        (output_dir / "normal_metrics.json").write_text(json.dumps({"coordinate_convention": "GT Blender world space compared directly with renderer world space", "normal_source": normal_source, "alpha_threshold": args.alpha_threshold, "summary_mean_over_frames": all_values, "frames": rows}, indent=2) + "\n", encoding="utf-8")
