...
本文记录 `PS-stage1-V3` 分支上 Stage1 V3.2 的推荐训练参数。V3.2 采用当前版本方案：光照方向固定使用 per-frame 参数表，每帧直接学习一个 raw light direction，渲染前 normalize；不使用 B-spline 或 control points；light smooth 使用一阶 / 二阶 L2 正则，并在正式训练前启用 16 相位 multistart 选择初始光照起点。
...
- `photometric_num_ctrl_points` 不参与 V3.2 light model，不要再用它调轨迹容量。
...
- 如果要做去正则对照，同时把 `lambda_photometric_light_smooth1` 和 `lambda_photometric_light_smooth2` 设为 `0.0`。
...
python -m scripts.train_stage1 \
  ...
  --model_path ${OUT_ROOT}/${SCENE}_stage1v32 \
  --images images \
...
```text
output/LH-staticv3.2/<scene>_stage1v32_static
```
...
不要通过 `photometric_num_ctrl_points` 调 V3.2，因为 V3.2 不再使用曲线控制点。

## 4. 不推荐参数

V3.2 不推荐继续使用曲线参数化相关配置：
...
- V3.2 active model 已固定为 per-frame light table。
- `photometric_num_ctrl_points` 对 V3.2 训练无效，继续保留只会造成实验记录混乱。
...
```bash
MODEL=output/LH-staticv3.2/${SCENE}_stage1v32_static
